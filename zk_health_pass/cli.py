"""
Command-Line Interface for ZK-HealthPass: Anonymous Selective-Disclosure Credential Agent.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="zk-proof-of-vaccination-pki", description="ZK-HealthPass: Anonymous Selective-Disclosure Credential Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  ZK-HEALTHPASS: ANONYMOUS SELECTIVE-DISCLOSURE CREDENTIAL AGENT")
        print(f"  Domain: Privacy-Preserving Federated Healthcare & FHE | Standard: W3C Verifiable Credentials & BBS+ Signatures")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting ZK-HealthPass: Anonymous Selective-Disclosure Credential Agent on http://{args.host}:{args.port}")
